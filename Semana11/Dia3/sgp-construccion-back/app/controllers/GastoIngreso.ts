...
//        GastoIngreso.build(objGastoIngreso)
       ...
       .then(( usuarios: any)=>{
           if(usuarios){
              Documento.findByPk(documento)
              .then((documentos: any)=>{
                    if(documentos){
                       GastoIngreso.build(objGastoIngreso).save()
...
export const updateGastoIngresoByPk = (req: Request, res: Response)=>{
       ...
       .then((objgastoingreso: any)=>{
            let rpta: Object = {ok: true, content: objgastoingreso};
            res.status(201).json(rpta);               
       })
       .catch((error: any)=>{
...
//        if({where: {!gasin_id: id_gastoingreso}})
}